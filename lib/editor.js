  ...
  var ed = exports.editor = {
    ...
    delete: function(ast,src,idx,args) {
      ...
      if (deleteRange) {
        ...
        var atStartOfUnsaveDelete = insideNodeStart && !isSaveToPartialDelete(insideNodeStart) && insideNodeStart.start === idx;
        var atEndOfUnsaveDelete = insideNodeEnd && !isSaveToPartialDelete(insideNodeEnd) && insideNodeEnd.end === endIdx;
        ...
        if (((insideNodeEnd && insideNodeEnd !== parent && !isSaveToPartialDelete(insideNodeEnd) && !atEndOfUnsaveDelete)
         || (insideNodeStart && insideNodeStart !== parent && !isSaveToPartialDelete(insideNodeStart) && !atStartOfUnsaveDelete))
         && insideNodeStart !== insideNodeEnd) return null;
        if ((insideNodeStart && parent.children.indexOf(insideNodeStart) === -1 && insideNodeStart !== parent)
         || (insideNodeEnd && parent.children.indexOf(insideNodeEnd) === -1 && insideNodeEnd !== parent)) return null;
      ...
      if (atStart && !atEnd && sexp.type === "string" && backward)
        return noDelete;

      if (atEnd && !atStart && sexp.type === "string" && !backward)
        return noDelete;
